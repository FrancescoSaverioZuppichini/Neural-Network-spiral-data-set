...
A Neural Network try to change its weights in order to decrease the error function, $E$. We define $\deta^l_j$ the error of neuron $j$ in layer $l$
\begin{equation}
	\label{eq:errorLayer}
	\delta^l_j = \frac{\delta E}{\delta z^l_j}
\end{equation}
Stringly speaking, $\delta^l_j$, is how much the error function change by changing the weighted input on that layer. %
...
We can express $\frac{\delta E}{\delta w^l_{jk}}$ by using equation \ref{eq:errorLayer}.
\begin{equation}
